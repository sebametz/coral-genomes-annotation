# *Agaricia agaricites* — jaAgaAgar1 / GCA_974392565.1

## Overview

| | |
|---|---|
| **Species** | *Agaricia agaricites* |
| **Family** | Agariciidae |
| **Order** | Scleractinia |
| **Assembly accession** | [GCA_974392565.1](https://www.ebi.ac.uk/ena/browser/view/GCA_974392565.1) |
| **ToLID** | jaAgaAgar1 |
| **Haplotype** | hap1 |
| **NCBI Taxonomy ID** | [89882](https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?id=89882) |
| **Annotation version** | v2 |
| **Annotation pipeline** | RepeatModeller;BRAKER3;Egg-NOG mapper;InterProScan — v2 |
| **Protein database** | R1: VARUS + db [Cnidaria + Scleractinia]; R2: VARUS + db [Scleractinia v2] — v2 |
| **BUSCO lineage** | cnidaria_odb12 |

## Assembly statistics

| Metric | Value |
|---|---|
| Total length | 546346346 bp |
| Scaffolds | 488 |
| N50 | 34245354 bp |
| GC content | 39.09% |
| Soft-masked | 50.37% |

## Gene annotation statistics

| Metric | Value |
|---|---|
| Genes | 29011 |
| Average gene length | 7042 bp |
| Transcripts per gene | 1.15 |
| Average transcript length | 1398 bp |
| Exons per transcript | 6.59 |
| Average exon length | 212 bp |
| Average intron length | 1136.5 bp |
| Single-exon transcripts | 10027 |

## Repeat content

| Class | % genome |
|---|---|
| Retroelements | 12.77% |
| DNA transposons | 5.30% |
| Rolling circles | 0.08% |
| Unclassified | 30.43% |
| Simple repeats | 1.16% |
| **Total interspersed** | **48.50%** |
| Soft-masked total | 50.37% |

## Annotation quality

| Tool | Result |
|---|---|
| BUSCO | C:98.6% [S:82.3%, D:16.3%] F:0.5% M:0.9% (n=3203) |
| OMARK completeness | 88.03% single, 8.79% duplicated, 3.18% missing |
| OMARK consistency | 64.95% consistent, 7.68% inconsistent, 27.36% unknown |

## Functional annotation

| Database | Coverage |
|---|---|
| EggNOG-mapper | 100.00% annotated |
| GO terms (EggNOG) | 51.50% of proteins |
| KEGG KO | 57.22% of proteins |
| COG categories | 90.82% of proteins |
| InterProScan domains | 100.00% of proteins |

## Downloads

| Resource | Link |
|---|---|
| Annotation files (this version, Zenodo) | [10.5281/zenodo.20721096](https://doi.org/10.5281/zenodo.20721096) |
| Assembly (ENA) | [GCA_974392565.1](https://www.ebi.ac.uk/ena/browser/view/GCA_974392565.1) |
| Assembly (NCBI) | [NCBI Datasets](https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_974392565.1/) |

### Files in this folder

| File | Description |
|---|---|
| `annotation.gff3.gz` | Gene models (GFF3 format) |
| `proteins.fa.gz` | Protein sequences (BRAKER3) |
| `transcripts.fa.gz` | Coding sequences (BRAKER3) |
| `eggnog.annotations.gz` | EggNOG-mapper functional annotation |
| `interproscan.tsv.gz` | InterProScan protein domain annotation |
| `repeatmasker.tbl` | RepeatMasker summary table |
| `coding_region_stats.txt` | Gene / transcript statistics |
| `busco_short_summary.txt` | BUSCO completeness summary |

## Changelog

**v2**

## Citation

If you use these annotations, please cite:

> Metz, S., Paulini, M., Rising, K. et al. Chromosome-level genomes of scleractinian corals: gene prediction and functional annotation. Sci Data (2026).

>https://doi.org/10.1038/s41597-026-07499-3

> Zenodo: [10.5281/zenodo.20721096](https://doi.org/10.5281/zenodo.20721096)

---
*README auto-generated on 2026-07-13 by generate_readmes.py*
